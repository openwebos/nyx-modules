...
static void fixture_setup(struct Fixture *f, gconstpointer userdata)
{
	...
	f->device = NULL;
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_device_open(NYX_DEVICE_SECURITY, "Main", &f->device));
...
static void test_create_aes_key(struct Fixture *f, gconstpointer userdata)
{
	...
	int index = -1;
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_create_aes_key(f->device, keylen, &index));
...
static void test_create_rsa_key(struct Fixture *f, gconstpointer userdata)
{
	...
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_create_rsa_key(f->device, keylen, &index));
	g_assert_cmpint(index, !=, -1); /* module assigned next (first) free index */
...
{
	nyx_security_aes_block_mode_t block_mode = (nyx_security_aes_block_mode_t)GPOINTER_TO_INT(userdata);
	int index = -1;
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_create_aes_key(f->device, 128, &index));
	...
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_crypt_aes(f->device, index, block_mode, 1, src, strlen(src) + 1, enc, &enclen, &ivlen));
	g_assert_cmpint(enclen, >, 0);

	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_crypt_aes(f->device, index, block_mode, 0, enc, enclen, dec, &declen, &ivlen));
...
static void test_crypt_rsa(struct Fixture *f, gconstpointer userdata)
{
	...
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_crypt_rsa(f->device, index, 1, src, strlen(src) + 1, enc, &enclen));
	g_assert_cmpint(enclen, >, 0);

	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_crypt_rsa(f->device, index, 0, enc, enclen, dec, &declen));
	g_assert_cmpint(declen, >, 0);
	g_assert_cmpstr(src, ==, dec);
}
...
{
	g_assert(userdata != NULL); /* "hash_type;expected_result" as user data pointer */
	...
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_init_hash(f->device, tokens[0]));
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_update_hash(f->device, msg, strlen(msg)));
	g_assert_cmpint(NYX_ERROR_NONE, ==, nyx_security_finalize_hash(f->device, hash));
