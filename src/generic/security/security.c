...
nyx_error_t nyx_module_open(nyx_instance_t i, nyx_device_t** d)
{
	...
	*d = (nyx_device_t*) calloc(sizeof(nyx_device_t), 1);
	...
	for (m = 0; m < sizeof(methods)/sizeof(methods[0]); ++m) {
		nyx_error_t result = nyx_module_register_method(i, *d, methods[m].method, methods[m].function);
...
nyx_error_t security_create_aes_key(nyx_device_handle_t d, int keylen, int *key_index)
{
	return aes_generate_key(keylen, key_index);
}

nyx_error_t security_aes_crypt(nyx_device_handle_t d, int key_index, nyx_security_aes_block_mode_t mode, int encrypt, const char* src, int srclen, char* dest, int *destlen, int *ivlen)
{
	return aes_crypt(key_index, encrypt, mode, src, srclen, dest, destlen, ivlen);
}

nyx_error_t security_create_rsa_key(nyx_device_handle_t d, int keylen, int *key_index)
{
	return rsa_generate_key(keylen, key_index);
}

nyx_error_t security_rsa_crypt(nyx_device_handle_t d, int key_index, int encrypt, const char* src, int srclen, char* dest, int *destlen)
{
...
nyx_error_t security_update_hash(nyx_device_handle_t d, const char *src, int srclen)
{
