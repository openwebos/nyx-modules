...
static void keystore_load_aeskey(GKeyFile *keyfile, gchar *key, struct keystore_t *keystore)
{
...
static void keystore_save_rsakey(gpointer key, gpointer value, gpointer user_data)
{
	const int key_index = GPOINTER_TO_INT(key);
...
static void keystore_load_rsakey(GKeyFile *keyfile, gchar *key, struct keystore_t *keystore)
{
	g_assert(NULL != keyfile);
	g_assert(NULL != key);
	g_assert(NULL != keystore);
	...
	if (length == 2) {
		...
		rsa_key->rsa = d2i_RSAPrivateKey(NULL, (const unsigned char**)&tmp, keybits_len);
		if (NULL != rsa_key->rsa) {
